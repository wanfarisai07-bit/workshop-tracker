...
export function VehicleScreen({ vehicleId }: { vehicleId: number }) {
  ...
  return (
    <div style={{ padding: '22px 26px', display: 'grid', gridTemplateColumns: '1fr 340px', gap: 18, maxWidth: 1060 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, minWidth: 0 }}>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', padding: 16 }}>
          <div style={{ font: '600 10.5px/1 var(--font-mono)', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--brand)', marginBottom: 14 }}>
            Status progress
          </div>
          {timeline.map((t, i) => (
            <div key={i} style={{ display: 'flex', gap: 12 }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 'none' }}>
                ...
                {t.hasLine && <span style={{ flex: 1, width: 2, minHeight: 22, background: 'var(--border)' }} />}
              </div>
              <div style={{ paddingBottom: 16 }}>
                <div style={{ font: '600 14px/1.3 var(--font-body)', color: t.labelColor }}>{t.label}</div>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', padding: 16 }}>
          ...
          {editingJobs ? (
            ...
            <>
              <div style={{ font: '400 11.5px/1.4 var(--font-body)', color: 'var(--fg3)', marginBottom: 8 }}>Click a job to move it on.</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, minWidth: 0 }}>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderLeft: '3px solid var(--brand)', borderRadius: 'var(--radius-md)', padding: 15 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
            <span style={{ font: '800 27px/1 var(--font-display)', color: 'var(--fg1)' }}>{v.plate}</span>
          </div>
          <div style={{ font: '500 11px/1 var(--font-mono)', color: 'var(--brand)', marginTop: 4 }}>{v.sheet}</div>
          <div style={{ display: 'grid', gridTemplateColumns: '78px 1fr', gap: '7px 8px', marginTop: 13, borderTop: '1px solid var(--border)', paddingTop: 12 }}>
            ...
            <span style={{ ...valueStyle, font: '500 13px/1.4 var(--font-mono)' }}>{v.odo} km</span>
          </div>
        </div>

        <div style={{ background: 'var(--blue-50)', borderLeft: '3px solid ' + st.color, padding: '13px 14px', borderRadius: 'var(--radius-xs)' }}>
          <span style={{ display: 'block', font: '600 10px/1 var(--font-mono)', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--fg3)' }}>
            Current stage
          </span>
          <span style={{ display: 'block', font: '600 15px/1.3 var(--font-body)', color: 'var(--fg1)', marginTop: 5 }}>{st.label}</span>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginTop: 10 }}>
            <span style={{ font: '700 20px/1 var(--font-display)', color: 'var(--fg1)', fontVariantNumeric: 'tabular-nums' }}>
              {v.stage === 'booked' ? 'Not in yet' : dur(v.at)}
            </span>
            <span style={{ font: '500 9.5px/1 var(--font-mono)', letterSpacing: '0.08em', textTransform: 'uppercase', color: over ? 'var(--amber-600)' : 'var(--fg3)' }}>
        ...
        {canAdvance && nextStage && (
          blockedByJobs ? (
            <>
              <PrimaryButton size="lg" disabled>Move to {nextStage.label}</PrimaryButton>
            ...
            <PrimaryButton size="lg" onClick={() => advanceVehicle(v.id)}>
              Move to {nextStage.label}
            </PrimaryButton>
          )
        )}
        {!canAdvance && (
          <div style={{ textAlign: 'center', font: '500 11px/1.4 var(--font-mono)', letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--fg3)', padding: 6 }}>
            Job sheet closed
          </div>
        )}
      </div>
...
const labelStyle = { font: '600 9.5px/1.4 var(--font-mono)', letterSpacing: '0.08em', textTransform: 'uppercase' as const, color: 'var(--fg3)' };
const valueStyle = { font: '400 13px/1.4 var(--font-body)', color: 'var(--fg1)' };
